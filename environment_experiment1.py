...
class SeamCarvingEnv(gym.Env):
    ...
    def __init__(self, img_path: str):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(self.line_count, self.line_length, 3), dtype=np.uint8)

        self.obs_image = self.fill_image_for_observations(self.img)

        print("In experiment 1 environment")

    def fill_image_for_observations(self, image):
        height, width, rgb = image.shape

        empty_matrix = np.full((height * 2, width * 3, 3), 255)
        empty_matrix[0:height, width:(width * 2), :] = image[0:height, 0:width, :]
